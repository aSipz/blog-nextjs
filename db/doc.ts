...
export const getOneDoc = async (db: Db, id: string) => db.collection('docs').findOne({ _id: id });

export const getDocsByFolder = async (db: Db, folderId: string) => db.collection('docs').find({ folder: folderId }).toArray();
...
export const updateOne = async (db: Db, id: string, updates: any) => {
    ...
    const updatedDoc = await getOneDoc(db, id);
    return updatedDoc;
};
